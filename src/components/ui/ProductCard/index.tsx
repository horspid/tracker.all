...
const ProductCard = ({ data, cardIndex }: ProductCardProps) => {
  const skeletonUrl =
  ...
  const viewImage = () => {
    ...
    } else return <img className="product_card__skeleton" src={skeletonUrl} />;
  };

  const checkRating = () => {
    if (userRate && userRate.user_rating) {
      return (
        <div className="product_card__rate product_card__rate_white">
          <RatingICO />
      ...
      if (data.rating.kp !== 0 || data.rating.imdb !== 0) {
        return (
          <div className="product_card__rate">
            <span>
  ...
  return (
    <article className={`product-card relative`} onClick={navigateHandle}>
      <div className="product-card__glass absolute z-10 px-10 py-5 text-white">
        {data.rating?.kp && data.rating.kp?.toFixed(1)}
      </div>
      <img
        src={
